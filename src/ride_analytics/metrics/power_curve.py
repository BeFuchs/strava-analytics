...
def ride_power_curve(
    ...
    Power is laid out on a 1 Hz grid over the ride's elapsed span; seconds
    without a sample (pauses, dropouts) count as 0 W — you are not pedaling
    while paused, so long-window bests stay honest.
    """
    if "power" not in df.columns or df["power"].notna().sum() == 0:
        return {}

    grid = df.set_index("timestamp")["power"].resample("1s").mean().fillna(0.0)
